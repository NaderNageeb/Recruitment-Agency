 ...
 if (isset($_GET['search'])) {
    ...
    $sql = "SELECT * FROM `tblcompany` c,`tbljob` j WHERE c.`COMPANYID`=j.`COMPANYID` AND j.`JOBSTATUS` = 'Open' AND COMPANYNAME LIKE '%" . $COMPANYNAME ."%' ORDER BY DATEPOSTED DESC" ;
    $mydb->setQuery($sql);
    ...
    if($today <= $end_date){
       ...
       $query_3 = mysqli_query($conn,$sqli_3); 
        ...
        echo '<td>'.date_format(date_create($result->end_date),'m/d/Y').'</td>';
