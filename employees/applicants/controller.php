...
global $mydb;
	if (isset($_POST['submit'])) {
		...
		$sql="UPDATE `tbljobregistration` SET `REMARKS`='{$remarks}',admin_app_1 = 3 ,PENDINGAPPLICATION=0,HVIEW=0,DATETIMEAPPROVED=NOW() WHERE `REGISTRATIONID`='{$id}'";
		$mydb->setQuery($sql);
		$cur = $mydb->executeQuery();
		...
		if ($cur) {
			...
			message("Massage Send Successfully.", "success");
